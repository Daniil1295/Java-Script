...
const CartItem = ({ item }) => {
  const { decreaseAmount, removeFromCart, increaseAmount } = useContext(CartContext);
  const { id, title, image, price, amount } = item;
  return <div className='flex gap-x-4 py-2 lg:px-6 border-b border-r-gray-200 w-full 
    ...
    gap-x-4'>
      ...
      <div className='w-full flex flex-col'>
        ...
        <div className='flex gap-x-2 h-[36px] text-sm'>
          ...
          {/* item final price */}
          {/* making the price at two decimals */}
          <div className='flex-1 flex  justify-end items-center font-medium 
           text-primary'>{`$  ${parseFloat(price * amount).toFixed(2)}`}</div>
        </div>
      </div>
    </div>
  </div>;
